...
class Runner:
    ...
    def review_pr(self, pr: GitPullRequest, pr_url: str, run_state: RunState):
        ...
        for rule in rules:
            ...
            if path_regex is not None:
                match_found, threads = self.check_diff(pr, pr_url, project, is_dry_run, pr_author,
                                                       threads, pr_review_state, file_diffs, rule, comment, comment_id, path_regex)
            ...
            if requeue is not None:
                if policy_evaluations is None:
                    ...
                    policy_evaluations_: list[PolicyEvaluationRecord] = self.policy_client.get_policy_evaluations(
                        project, f'vstfs:///CodeReview/CodeReviewId/{project_id}/{pr.pull_request_id}')
    ...
    def check_votes(self,
        ...
        iterations: list[GitPullRequestIteration] = self.git_client.get_pull_request_iterations(
            repository_id, pr.pull_request_id, project=project, include_commits=False)
        if len(iterations) == 0:
    ...
    def check_diff(self,
                   ...
                   project: str,
                   is_dry_run: bool,
                   pr_author: IdentityRef,
                   threads: Optional[list[GitPullRequestCommentThread]],
                   pr_review_state: PrReviewState,
        ...
        for file_diff in file_diffs:
            ...
            if diff_regex is None:
                ...
                if file_diff.diff is not None:
                    ...
                    for block in file_diff.diff['blocks']:
                        ...
                        for start_line_num, line in enumerate(block['mLines'], start=first_line_num):
                            local_match_found, threads = self.check_line_diff(
                                pr, pr_url, project, is_dry_run, pr_author, threads, rule, pr_review_state, comment, comment_id, diff_regex, file_diff, start_line_num, line)
                            match_found = match_found or local_match_found

                        if diff_regex.flags & re.MULTILINE:
                            text = '\n'.join(block['mLines'])
                            local_match_found, threads = self.check_text_diff(
                                pr, pr_url, project, is_dry_run, pr_author, threads, rule, pr_review_state, file_diff, text, comment, comment_id, diff_regex, first_line_num)
                            match_found = match_found or local_match_found
                if file_diff.contents is not None:
                    ...
                    if diff_regex.flags & re.MULTILINE:
                        local_match_found, threads = self.check_text_diff(
                            pr, pr_url, project, is_dry_run, pr_author, threads, rule, pr_review_state, file_diff, file_diff.contents, comment, comment_id, diff_regex, first_line_num)
                        match_found = match_found or local_match_found
        return match_found, threads

    def add_optional_reviewers(
            self,
            pr: GitPullRequest,
            pr_url: str,
            project: str,
            pr_author: IdentityRef,
            is_dry_run: bool,
            reviewers: list[IdentityRefWithVote],
            optional_reviewers: Collection[str]):
        for optional_reviewer in optional_reviewers:
            ...
            if not is_already_optional:
                if not is_dry_run:
                    ...
                    repository_id = pr.repository.id  # type: ignore
                    self.git_client.create_pull_request_reviewer(
                        req, repository_id, pr.pull_request_id, reviewer_id=optional_reviewer, project=project)
                else:
                    self.logger.info("Would add optional reviewer: %s\nTitle: \"%s\"\nBy %s (%s)\n%s",
                                     optional_reviewer, pr.title, pr_author.display_name, pr_author.unique_name, pr_url)

    def add_required_reviewers(
            self,
            pr: GitPullRequest,
            pr_url: str,
            project: str,
            ...
            is_dry_run: bool,
            reviewers: list[IdentityRefWithVote],
            required_reviewers: str | Collection[str]):
        ...
        for required_reviewer in required_reviewers:
            ...
            if not is_already_required:
                if not is_dry_run:
                    ...
                    repository_id = pr.repository.id  # type: ignore
                    self.git_client.create_pull_request_reviewer(
                        req, repository_id, pr.pull_request_id, reviewer_id=required_reviewer, project=project)
                else:
    ...
    def check_text_diff(self,
        ...
        for m in diff_regex.finditer(text):
            ...
            local_match_found, threads = self.handle_diff_found(pr, pr_url, project, is_dry_run, pr_author, threads, rule, pr_review_state,
                                                                matching_text, comment, comment_id, file_diff, start_line_num, start_offset, end_line_num, end_offset)
            match_found = match_found or local_match_found
        return match_found, threads

    def check_line_diff(self,
                        pr: GitPullRequest,
                        pr_url: str,
                        project: str,
                        is_dry_run: bool,
    ...
    def get_diffs(self, pr: GitPullRequest, pr_url: str) -> list[FileDiff]:
        ...
        diffs: GitCommitDiffs = self.git_client.get_commit_diffs(
            repository_id,
            project,
            ...
            if not item.get('isFolder'):
                ...
                try:
                    ...
                    self.logger.exception("Failed to get diff for \"%s\" for \"%s\".\n  Title: \"%s\"\n  URL: %s",
                                          change_type, modified_path, pr.title, pr_url)

        return result

    def find_comment(
            self,
            threads: list[GitPullRequestCommentThread],
            comment: str,
            comment_id: Optional[str] = None,
            path: Optional[str] = None,
            start_line_num: Optional[int] = None) -> Optional[CommentSearchResult]:
        result = None
        ...
        for thread in threads:
            comments: Collection[Comment] = thread.comments  # type: ignore
            # We could filter by `thread.status` (can be 'active', 'unknown', ...),
            # but we want to find resolved threads too and reactivate them, if
            # necessary.
            if path is not None:
    ...
    def send_comment(
            ...
            threads: list[GitPullRequestCommentThread],
            pr_review_state: PrReviewState,
            thread_context: Optional[CommentThreadContext] = None,
            status='active',
            comment_id: Optional[str] = None,
        ...
        if current_count >= comment_count_limit:
            self.logger.debug(
                "Skipping comment \"%s\" because the limit of %d comments has been reached for \"%s\".\n  URL: %s",
                comment,
                comment_count_limit,
                pr.title,
                pr_url)
        ...
        if not is_dry_run:
            self.logger.info(
                ...
                position_phrase,
                pr.title,
                pr_author.display_name,
                pr_author.unique_name,
                pr_url)
            project = self.config['project']
            repository_id = pr.repository.id  # type: ignore
            ...
            self.logger.info("Would comment: \"%s\"%s\nTitle: \"%s\"\nBy %s (%s)\n%s", comment,
                             position_phrase, pr.title, pr_author.display_name, pr_author.unique_name, pr_url)
    ...
    def update_comment(
            ...
            comment: str,
            comment_id: Optional[str],
            existing_comment_info: CommentSearchResult,
            status='active',
            matching_text: Optional[str] = None):
        ...
        if thread.status != status:
            ...
            if not is_dry_run:
                self.logger.info("CHANGING THREAD STATUS: \"%s\" for comment: \"%s\"\nTitle: \"%s\"\nBy %s (%s)\n%s",
                                 status, comment, pr.title, pr_author.display_name, pr_author.unique_name, pr_url)
                project = self.config['project']
                repository_id = pr.repository.id  # type: ignore
    ...
    def delete_comments(self, pr, pr_url: str, project, repository_id,
                        delete_comment: Optional[str]) -> Optional[list[GitPullRequestCommentThread]]:
        ...
        for policy_evaluation in policy_evaluations:
            ...
            if all(self.matcher_checker.is_check_match(rule_policy_check, policy_evaluation) for rule_policy_check in requeue):
                ...
                if evaluation_id is None:
                    self.logger.warning(
                        "Cannot requeue check for \"%s\" because no evaluation ID was found for policy evaluation: %s",
                        name,
                        policy_evaluation)
                    break
                if not is_dry_run:
